...
typedef struct {
    ...
    member_coverage_t *current_benefit_coverage;
    coverage_benefit_t *current_benefit;
...
static int report_capacity_exceeded(
    const char *item,
    size_t limit,
...
)
{
    fprintf(
        stderr,
        "scribe stitch coverage: capacity exceeded item=%s limit=%zu aggregate=%s source_drop=%s\n",
        item == NULL ? "(unknown)" : item,
        limit,
        aggregate_key == NULL || aggregate_key[0] == '\0' ? "(none)" : aggregate_key,
        source_drop_id == NULL || source_drop_id[0] == '\0' ? "(none)" : source_drop_id
    );
    return X12_ERR_NO_MEMORY;
}

static void clear_current_benefit(coverage_state_t *state)
{
    if (state == NULL) {
        return;
    }

    state->current_benefit_coverage = NULL;
    state->current_benefit = NULL;
...
)
{
    ...
    *out = (size_t)value;
    if ((unsigned long long)*out != value) {
...
)
{
    ...
    if (separator == NULL) {
        scribe_copy_cstr(last_name_or_org, last_len, name);
        return;
    }
    ...
    scribe_copy_cstr(first_name, first_len, separator + 1);
}
...
)
{
    ...
    scribe_copy_cstr(coverage->member_name_token, sizeof(coverage->member_name_token), token);
    return X12_OK;
}
...
static int find_or_add_coverage(
    coverage_state_t *state,
    const char *member_id,
    const char *member_id_token,
    member_coverage_t **out
)
{
    ...
    *out = NULL;
    ...
    if (coverage != NULL) {
        if (coverage->member_id_token[0] == '\0') {
            scribe_copy_cstr(coverage->member_id_token, sizeof(coverage->member_id_token), member_id_token);
        }
        if (state->include_phi && coverage->member_id[0] == '\0') {
            scribe_copy_cstr(coverage->member_id, sizeof(coverage->member_id), member_id);
        }
        *out = coverage;
        return X12_OK;
    }
...
)
{
    ...
    scribe_copy_cstr(source_event->event_type, sizeof(source_event->event_type), event_type);
    return X12_OK;
}
...
{
    coverage_update_batch_t *notification_batch = NULL;
...
)
{
    ...
    if (batch == NULL) {
        ...
        scribe_copy_cstr(batch->source_drop_id, sizeof(batch->source_drop_id), source_drop_id);
...
static int apply_pending_enrollment(member_coverage_t *coverage, const coverage_state_t *state)
{
    ...
    scribe_copy_cstr(coverage->relationship_code, sizeof(coverage->relationship_code), state->pending_relationship_code);
    scribe_copy_cstr(
        coverage->enrollment_maintenance_type_code,
...
)
{
    ...
    scribe_copy_cstr(state->current_member_id, sizeof(state->current_member_id), member_id);
    scribe_copy_cstr(state->current_member_id_token, sizeof(state->current_member_id_token), member_id_token);
    clear_current_benefit(state);
    rc = find_or_add_coverage(state, member_id, member_id_token, &coverage);
    if (rc != X12_OK || coverage == NULL) {
        return rc == X12_OK ? X12_ERR_NO_MEMORY : rc;
    }
    ...
    if (state->current_payer_id_token[0] != '\0' || state->current_payer_id[0] != '\0') {
        if (state->include_phi) {
            scribe_copy_cstr(coverage->payer_id, sizeof(coverage->payer_id), state->current_payer_id);
        }
        scribe_copy_cstr(coverage->payer_id_token, sizeof(coverage->payer_id_token), state->current_payer_id_token);
    }
...
)
{
    ...
    if (strcmp(qualifier, "348") == 0) {
        scribe_copy_cstr(coverage->coverage_effective_date, sizeof(coverage->coverage_effective_date), value);
    } else if (strcmp(qualifier, "349") == 0) {
        scribe_copy_cstr(coverage->coverage_termination_date, sizeof(coverage->coverage_termination_date), value);
    } else {
        scribe_copy_cstr(
            coverage->last_coverage_date_qualifier,
        ...
        scribe_copy_cstr(coverage->last_coverage_date, sizeof(coverage->last_coverage_date), value);
    }
...
)
{
    ...
    if (!is_member) {
        ...
        scribe_copy_cstr(state->current_payer_id_token, sizeof(state->current_payer_id_token), id_token);
        return X12_OK;
    }

    scribe_copy_cstr(state->current_member_id, sizeof(state->current_member_id), raw_id);
    scribe_copy_cstr(state->current_member_id_token, sizeof(state->current_member_id_token), id_token);
    clear_current_benefit(state);
    rc = find_or_add_coverage(state, raw_id, id_token, &coverage);
    if (rc != X12_OK || coverage == NULL) {
        return rc == X12_OK ? X12_ERR_NO_MEMORY : rc;
    }
    if (state->include_phi) {
        scribe_copy_cstr(coverage->member_id, sizeof(coverage->member_id), raw_id);
        scribe_copy_cstr(coverage->payer_id, sizeof(coverage->payer_id), state->current_payer_id);
    }
    scribe_copy_cstr(coverage->member_id_token, sizeof(coverage->member_id_token), id_token);
    scribe_copy_cstr(coverage->payer_id_token, sizeof(coverage->payer_id_token), state->current_payer_id_token);
...
)
{
    ...
    scribe_copy_cstr(coverage->date_of_birth_token, sizeof(coverage->date_of_birth_token), dob_token);
    if (state->include_phi) {
...
)
{
    ...
    if (strcmp(event_type, "EligibilityInquiryDateRecorded") == 0) {
        if (strcmp(qualifier, "291") == 0) {
            scribe_copy_cstr(state->current_inquiry_date, sizeof(state->current_inquiry_date), value);
        }
    } else if (strcmp(date_scope, "benefit") == 0 &&
               state->has_current_benefit &&
               state->current_benefit_coverage == coverage &&
               state->current_benefit != NULL) {
        benefit = state->current_benefit;
        if (strcmp(qualifier, "346") == 0) {
            scribe_copy_cstr(benefit->effective_date, sizeof(benefit->effective_date), value);
        } else if (strcmp(qualifier, "347") == 0) {
            scribe_copy_cstr(benefit->termination_date, sizeof(benefit->termination_date), value);
        }
    } else if (strcmp(qualifier, "291") == 0) {
        scribe_copy_cstr(state->current_response_as_of_date, sizeof(state->current_response_as_of_date), value);
    }
...
)
{
    ...
    scribe_copy_cstr(request->eligibility_id, sizeof(request->eligibility_id), state->current_eligibility_id);
    if (state->include_phi) {
        scribe_copy_cstr(request->payer_id, sizeof(request->payer_id), state->current_payer_id);
    }
    scribe_copy_cstr(request->payer_id_token, sizeof(request->payer_id_token), state->current_payer_id_token);
    scribe_copy_cstr(request->service_type_code, sizeof(request->service_type_code), service_type_code);
    scribe_copy_cstr(request->inquiry_date, sizeof(request->inquiry_date), state->current_inquiry_date);
...
)
{
    ...
    scribe_copy_cstr(benefit->eligibility_id, sizeof(benefit->eligibility_id), state->current_eligibility_id);
    if (state->include_phi) {
        scribe_copy_cstr(benefit->payer_id, sizeof(benefit->payer_id), state->current_payer_id);
    }
    scribe_copy_cstr(benefit->payer_id_token, sizeof(benefit->payer_id_token), state->current_payer_id_token);
...
static int coverage_stitcher_run_passes(
    const coverage_stitcher_input_t *input,
    coverage_state_t *state,
...
)
{
    ...
    if (input == NULL || state == NULL || journal == NULL) {
        return X12_ERR_INVALID_ARGUMENT;
    }
...
int coverage_stitcher_stitch(const coverage_stitcher_input_t *input)
{
    ...
    rc = coverage_stitcher_run_passes(input, state, &journal, &event_count);
