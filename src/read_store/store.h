...
#define SCRIBE_STORE_NOTIFICATION_ID_MAX 384u
#define SCRIBE_STORE_PAYLOAD_MAX 4096u
...
typedef struct
{
    ...
    char event_type[SCRIBE_STORE_TYPE_MAX];
    char aggregate_type[SCRIBE_STORE_TYPE_MAX];
    char source_drop_id[SCRIBE_STORE_ID_MAX];
    char run_id[SCRIBE_STORE_ID_MAX];
    char source_run_id[SCRIBE_STORE_ID_MAX];
    size_t aggregate_version_count;
    char payload_json[SCRIBE_STORE_PAYLOAD_MAX];
} scribe_outbox_notification_t;
...
typedef int (*scribe_aggregate_version_cb)(
    const char *aggregate_id,
    size_t version,
    const char *state_json,
...
typedef int (*scribe_outbox_notification_cb)(
