...
{
    return mapper->in_service_line ? "ClaimLineDateRecorded" : "ClaimDateRecorded";
}

static int add_current_claim_fields(x12_mapper_837_t *mapper)
{
    ...
    int clear_after_flush)
{
    ...
    if (rc == X12_OK && clear_after_flush)
    {
        buffer->present = 0;
    }
    return rc;
...
static int flush_claim_context_references(x12_mapper_837_t *mapper)
{
    TRY(flush_buffered_nm1_reference(
        mapper, &mapper->billing_provider,
        "ClaimReferencedBillingProvider", TOK_PROVIDER_ID, 0));
    TRY(flush_buffered_nm1_reference(
        mapper, &mapper->subscriber,
        "ClaimReferencedSubscriber", TOK_MEMBER_ID, 0));
    TRY(flush_buffered_nm1_reference(
        mapper, &mapper->patient,
        "ClaimReferencedPatient", TOK_PATIENT_ID, 0));
    return flush_buffered_nm1_reference(
        mapper, &mapper->rendering_provider,
        "ClaimReferencedRenderingProvider", TOK_PROVIDER_ID, 1);
}
...
static int flush_claim_context_taxonomies(x12_mapper_837_t *mapper)
{
    TRY(flush_buffered_provider_taxonomy(
        mapper, &mapper->billing_provider_taxonomy, "billing_provider", 0));
    return flush_buffered_provider_taxonomy(
        mapper, &mapper->rendering_provider_taxonomy, "rendering_provider", 1);
}
...
static int flush_claim_party_context(x12_mapper_837_t *mapper)
{
    TRY(flush_buffered_claim_context(mapper, &mapper->subscriber_info, write_subscriber_information));
    TRY(flush_buffered_demographics(mapper, &mapper->subscriber_demographics, X12_837_PARTY_SUBSCRIBER));
    TRY(flush_buffered_claim_context(mapper, &mapper->patient_info, write_patient_information));
    return flush_buffered_demographics(mapper, &mapper->patient_demographics, X12_837_PARTY_PATIENT);
}
    ...
    const x12_segment_t *seg)
{
    ...
    TRY(event_writer_begin_event(w, "ClaimReferenceRecorded", seg));
    TRY(add_current_claim_fields(mapper));
    TRY(event_writer_add_cstr(w, "reference_scope", current_loop_scope(mapper)));
    TRY(event_writer_add_str(w, "service_line_number", mapper->current_service_line_number));
    ...
    for (i = 0; i < seg->element_count; i++)
    {
        ...
        if (diagnosis_is_principal(qualifier))
        {
            rc = normalize_diagnosis_code(
                raw_code, principal_buffer, sizeof(principal_buffer), &principal_diagnosis);
            if (rc != X12_OK)
            {
                return rc;
            }
    ...
    x12_str_t raw_component)
{
    ...
    component_count = split_components(
        raw_component, mapper->component_sep, components,
    ...
    TRY(event_writer_begin_event(w, "ClaimHealthcareCodeRecorded", seg));
    TRY(add_current_claim_fields(mapper));
    TRY(event_writer_add_cstr(w, "healthcare_code_kind", healthcare_code_kind(components[0])));
    TRY(event_writer_add_str(w, "healthcare_code_qualifier", components[0]));
    ...
    const x12_segment_t *seg)
{
    ...
    {
        TRY(write_healthcare_code_recorded(mapper, seg, seg->elements[i]));
    }
    return X12_OK;
}
    ...
    const x12_segment_t *seg)
{
    ...
    if (diagnosis_pointers.len > 0u)
    {
        diagnosis_pointer_count = split_components(
            diagnosis_pointers, mapper->component_sep, diagnosis_pointer_values,
            sizeof(diagnosis_pointer_values) / sizeof(diagnosis_pointer_values[0]));
    }

    split_procedure_components(
        procedure_element, mapper->component_sep,
    ...
    TRY(event_writer_add_str_array(
        w, "procedure_modifiers", procedure_modifiers, procedure_modifier_count));
    TRY(event_writer_add_str(w, "charge_amount", charge_amount));
    TRY(event_writer_add_str(w, "unit_measure_code", unit_measure_code));
    TRY(event_writer_add_str(w, "unit_count", unit_count));
    TRY(event_writer_add_str_array(
        w, "diagnosis_pointers", diagnosis_pointer_values, diagnosis_pointer_count));
    TRY(event_writer_add_str_array(w, "raw_elements", seg->elements, seg->element_count));
    return event_writer_end_event(w);
}
...
int x12_mapper_837_on_segment(const x12_segment_t *seg, void *user)
{
    ...
    if (x12_str_eq_cstr(seg->tag, "NM1") && seg->element_count > 0u)
    {
        ...
        {
            mapper->current_provider_context = provider_context_from_nm1(seg->elements[0]);
            return write_claim_scoped_nm1_reference(mapper, seg, "ClaimReferencedReferringProvider", TOK_PROVIDER_ID);
        }
        if (x12_str_eq_cstr(seg->elements[0], "DQ"))
        {
            mapper->current_provider_context = provider_context_from_nm1(seg->elements[0]);
            return write_claim_scoped_nm1_reference(mapper, seg, "ClaimReferencedSupervisingProvider", TOK_PROVIDER_ID);
        }
        ...
        {
            mapper->current_provider_context = provider_context_from_nm1(seg->elements[0]);
            return write_claim_scoped_nm1_reference(mapper, seg, "ClaimReferencedAttendingProvider", TOK_PROVIDER_ID);
        }
        if (x12_str_eq_cstr(seg->elements[0], "72"))
        {
            mapper->current_provider_context = provider_context_from_nm1(seg->elements[0]);
            return write_claim_scoped_nm1_reference(mapper, seg, "ClaimReferencedOperatingProvider", TOK_PROVIDER_ID);
        }
    ...
    {
        TRY(write_diagnosis_observed(mapper, seg));
