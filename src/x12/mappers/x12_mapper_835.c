    ...
    const x12_segment_t *seg)
{
    ...
    TRY(event_writer_add_str(w, "transaction_handling_code", element_or_empty(bpr, 0)));
    TRY(event_writer_add_str(w, "payment_amount", element_or_empty(bpr, 1)));
    TRY(event_writer_add_str(w, "credit_debit_flag", element_or_empty(bpr, 2)));
    TRY(event_writer_add_str(w, "payment_method_code", element_or_empty(bpr, 3)));
    TRY(event_writer_add_str(w, "payment_date", element_or_empty(bpr, 15)));
    ...
    const x12_segment_t *seg)
{
    ...
    TRY(event_writer_add_str(w, "remittance_id", mapper->remittance_id));
    TRY(event_writer_add_str(w, "entity_identifier_code", element_or_empty(seg, 0)));
    TRY(add_phi_str(w, "name", element_or_empty(seg, 1)));
    ...
    TRY(event_writer_add_str(w, "id_qualifier", element_or_empty(seg, 2)));
    TRY(add_tokenized_or_phi(w, "id_value", token_type, element_or_empty(seg, 3)));
    TRY(add_phi_token(w, "id_value_token", token_type, element_or_empty(seg, 3)));
    return event_writer_end_event(w);
}
    ...
    const x12_segment_t *seg)
{
    ...
    TRY(event_writer_add_str(w, "claim_status_code", element_or_empty(seg, 1)));
    TRY(event_writer_add_str(w, "total_charge_amount", element_or_empty(seg, 2)));
    TRY(event_writer_add_str(w, "paid_amount", element_or_empty(seg, 3)));
    TRY(event_writer_add_str(w, "patient_responsibility_amount", element_or_empty(seg, 4)));
    TRY(event_writer_add_str(w, "claim_filing_indicator_code", element_or_empty(seg, 5)));
    TRY(add_tokenized_or_phi(
        w, "payer_claim_control_number", TOK_PAYER_CLAIM_CONTROL_NUMBER, element_or_empty(seg, 6)));
    TRY(add_phi_token(
        w, "payer_claim_control_number_token", TOK_PAYER_CLAIM_CONTROL_NUMBER, element_or_empty(seg, 6)));
    TRY(event_writer_add_str(w, "facility_type_code", element_or_empty(seg, 7)));
    TRY(event_writer_add_str(w, "claim_frequency_type_code", element_or_empty(seg, 8)));
    return event_writer_end_event(w);
}
    ...
    const x12_segment_t *seg)
{
    ...
    split_first_component(
        element_or_empty(seg, 0), mapper->component_sep, &procedure_qualifier, &procedure_code);
    ...
    TRY(event_writer_add_str(w, "line_charge_amount", element_or_empty(seg, 1)));
    TRY(event_writer_add_str(w, "line_paid_amount", element_or_empty(seg, 2)));
    TRY(event_writer_add_str(w, "paid_service_unit_count", element_or_empty(seg, 4)));
    return event_writer_end_event(w);
}
    ...
    const x12_segment_t *seg)
{
    ...
    TRY(event_writer_add_str(w, "service_line_number", current_service_line_number(mapper)));
    TRY(event_writer_add_str(w, "adjustment_group_code", element_or_empty(seg, 0)));
    ...
    const x12_segment_t *seg)
{
    ...
    TRY(event_writer_add_str(w, "date_value", element_or_empty(seg, 1)));
    return event_writer_end_event(w);
}
