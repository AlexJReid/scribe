...
static int compress_test_file_zstd(const char *input_path, const char *out_path)
{
    FILE *fp;
    ...
    REQUIRE(fp != NULL);
    REQUIRE(fseek(fp, 0L, SEEK_END) == 0);
    input_len = ftell(fp);
    REQUIRE(input_len >= 0L);
    REQUIRE(fseek(fp, 0L, SEEK_SET) == 0);

    input = (unsigned char *)malloc((size_t)input_len);
    REQUIRE(input != NULL);
    read_len = fread(input, 1u, (size_t)input_len, fp);
    REQUIRE(read_len == (size_t)input_len);
    REQUIRE(fclose(fp) == 0);

    output_cap = ZSTD_compressBound((size_t)input_len);
    output = (unsigned char *)malloc(output_cap);
    REQUIRE(output != NULL);
    output_len = ZSTD_compress(
        output,
        output_cap,
        input,
        (size_t)input_len,
    ...
    REQUIRE(!ZSTD_isError(output_len));

    fp = fopen(out_path, "wb");
    REQUIRE(fp != NULL);
    REQUIRE(fwrite(output, 1u, output_len, fp) == output_len);
    REQUIRE(fclose(fp) == 0);
...
static int test_journal_dictionary_allows_dynamic_fields(void)
{
    char journal_path[512];
    char value[128];
    int bool_value = 0;
    ...
    REQUIRE(make_path(journal_path, sizeof(journal_path), TEST_OUTPUT_DIR, "dynamic_fields.journal") == 0);
    (void)remove(journal_path);
    ...
    journal_record_builder_init(&builder);
    REQUIRE_OK(journal_record_builder_reset(&builder));
    REQUIRE_OK(journal_record_add_cstring(&builder, "event_type", "DynamicEvent"));
    REQUIRE_OK(journal_record_add_cstring(&builder, "custom_field", "custom-value"));
    REQUIRE_OK(journal_record_add_bool(&builder, "custom_bool", 1));
    REQUIRE_OK(journal_record_add_u64(&builder, "custom_number", 42u));
    REQUIRE_OK(journal_write_record(fp, &builder, NULL, NULL));
    journal_record_builder_free(&builder);
    REQUIRE(fclose(fp) == 0);
    ...
    REQUIRE(event.record_len > 0u);
    REQUIRE(journal_event_get_string(&event, "event_type", value, sizeof(value)) == 1);
    REQUIRE_STR(value, "DynamicEvent");
    REQUIRE(journal_event_get_string(&event, "custom_field", value, sizeof(value)) == 1);
    REQUIRE_STR(value, "custom-value");
    REQUIRE(journal_event_get_bool(&event, "custom_bool", &bool_value) == 1);
    REQUIRE(bool_value == 1);
    REQUIRE(journal_event_get_number_text(&event, "custom_number", value, sizeof(value)) == 1);
    REQUIRE_STR(value, "42");
    REQUIRE_OK(journal_reader_next(&reader, &event));
    REQUIRE(event.record_len == 0u);
    REQUIRE_OK(journal_reader_close(&reader));
...
static int test_journal_reader_reads_partition_directory(void)
{
    ...
    REQUIRE(snprintf(earlier_zst_path, sizeof(earlier_zst_path), "%s.zst", earlier_path) > 0);
    ...
    (void)remove(earlier_zst_path);

    REQUIRE(write_test_journal_segment(later_path, "LaterDrop") == 0);
    REQUIRE(write_test_journal_segment(earlier_path, "EarlierDrop") == 0);
    REQUIRE(compress_test_file_zstd(earlier_path, earlier_zst_path) == 0);
    REQUIRE(remove(earlier_path) == 0);
    ...
    REQUIRE_STR(event.segment_path, "20260601/drop-a.journal.zst");
    ...
    REQUIRE(event.record_len > 0u);
    REQUIRE(event.segment_path != NULL);
    REQUIRE_STR(event.segment_path, "20260602/drop-b.journal");
    REQUIRE(journal_event_get_string(&event, "event_type", event_type, sizeof(event_type)) == 1);
...
int main(void)
{
    ...
    REQUIRE(test_journal_dictionary_allows_dynamic_fields() == 0);
    return 0;
}
