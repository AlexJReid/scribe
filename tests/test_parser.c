...
static void copy_test_cstr(char *out, size_t out_len, const char *value)
{
    size_t len;
...
typedef struct {
    scribe_outbox_notification_t notifications[16];
    size_t count;
} parser_outbox_seen_t;
...
} parser_aggregate_version_seen_t;

typedef int (*parser_mapper_fn_t)(x12_document_t *doc, event_writer_t *writer);

static int collect_parser_outbox_notification(
    const scribe_outbox_notification_t *notification,
    void *user
)
{
    parser_outbox_seen_t *seen = (parser_outbox_seen_t *)user;

    if (seen == NULL || notification == NULL) {
        return X12_ERR_INVALID_ARGUMENT;
    }
    if (seen->count >= sizeof(seen->notifications) / sizeof(seen->notifications[0])) {
        return X12_ERR_NO_MEMORY;
    }
    seen->notifications[seen->count++] = *notification;
    return X12_OK;
}

static int collect_parser_aggregate_version(
...
{
    parser_aggregate_version_seen_t *seen = (parser_aggregate_version_seen_t *)user;

    (void)state_json;
    (void)updated_by_event_id;
    if (seen == NULL) {
        return X12_ERR_INVALID_ARGUMENT;
    }
    if (seen->count == 0u) {
        copy_test_cstr(seen->aggregate_id, sizeof(seen->aggregate_id), aggregate_id);
        seen->version = version;
        copy_test_cstr(seen->source_drop_id, sizeof(seen->source_drop_id), source_drop_id);
    }
    seen->count++;
    return X12_OK;
}
...
static int expect_phi_source_drop_error(
    parser_mapper_fn_t map_fn,
    const char *transaction_type,
    const char *segments
)
{
    ...
    REQUIRE(buffer != NULL);
    memcpy(buffer, segments, len + 1u);

    memset(&doc, 0, sizeof(doc));
    doc.delimiters.element_sep = '*';
    doc.delimiters.component_sep = ':';
    doc.delimiters.segment_term = '~';
    doc.buffer = buffer;
    doc.buffer_len = len;

    fp = tmpfile();
    REQUIRE(fp != NULL);
    REQUIRE_OK(event_writer_open_stream(&writer, fp, "synthetic.edi", transaction_type));
    phi_vault_init(&phi_vault);
    event_writer_set_phi_vault(&writer, &phi_vault);
    event_writer_set_include_phi(&writer, 1);

    rc = map_fn(&doc, &writer);
    REQUIRE(rc == X12_ERR_INVALID_ARGUMENT);
...
static int test_mappers_preserve_phi_mapping_errors(void)
{
    REQUIRE(expect_phi_source_drop_error(
                x12_map_270_document,
                "270",
                "NM1*IL*1*DOE*JANE****MI*SUB123~"
            ) == 0);
    REQUIRE(expect_phi_source_drop_error(
                x12_map_271_document,
                "271",
                "NM1*IL*1*DOE*JANE****MI*SUB123~"
            ) == 0);
    REQUIRE(expect_phi_source_drop_error(
                x12_map_834_document,
                "834",
                "NM1*IL*1*DOE*JANE****MI*SUB123~"
            ) == 0);
    REQUIRE(expect_phi_source_drop_error(
                x12_map_835_document,
                "835",
                "CLP*CLM123*1*10*8*2*12*11*1~"
            ) == 0);
    REQUIRE(expect_phi_source_drop_error(
                x12_map_837_document,
                "837",
                "CLM*CLM123*10***11:B:1*Y*A*Y*I~"
            ) == 0);
...
static int test_journal_builder_file_list(void)
{
    ...
    REQUIRE_OK(journal_builder_input_add_list(&journal_input, "837", list_path));
...
static int test_stroke_balance_projection_from_journal(void)
{
    ...
    parser_outbox_seen_t outbox_seen;
    parser_aggregate_version_seen_t changed_versions;
    ...
    memset(&outbox_seen, 0, sizeof(outbox_seen));
    REQUIRE_OK(scribe_store_each_outbox_notification_after(
                &read_store,
                0,
                16u,
                collect_parser_outbox_notification,
                &outbox_seen
            ));
    REQUIRE(outbox_seen.count == 8u);
    REQUIRE_STR(outbox_seen.notifications[0].event_type, "SourceDropAggregatesRecorded");
    REQUIRE_STR(outbox_seen.notifications[0].aggregate_type, "claim");
    REQUIRE_STR(outbox_seen.notifications[0].run_id, "stitch-test-run");
    REQUIRE_STR(outbox_seen.notifications[0].source_run_id, "ingest-test-run");
    REQUIRE(outbox_seen.notifications[0].aggregate_version_count == 1u);
    REQUIRE(strstr(outbox_seen.notifications[0].payload_json, "\"ok\":true") != NULL);
    REQUIRE(strstr(outbox_seen.notifications[0].payload_json, "CLM-STROKE") == NULL);
    REQUIRE(strstr(outbox_seen.notifications[0].payload_json, "PAYER-STROKE") == NULL);
    REQUIRE(strstr(outbox_seen.notifications[0].payload_json, "PAT-STROKE") == NULL);
    REQUIRE(strstr(outbox_seen.notifications[0].payload_json, "SUB-STROKE") == NULL);
    REQUIRE(strstr(outbox_seen.notifications[0].payload_json, "REID") == NULL);
    REQUIRE(strstr(outbox_seen.notifications[0].payload_json, "ALEX") == NULL);
    memset(&changed_versions, 0, sizeof(changed_versions));
    REQUIRE_OK(scribe_store_each_claim_aggregate_version_for_source_drop(
                &read_store,
                "835:000000102:102:0001",
                collect_parser_aggregate_version,
                &changed_versions
            ));
    REQUIRE(changed_versions.count == 1u);
    REQUIRE_STR(changed_versions.aggregate_id, "claim:8259c238232f9585e95fc8f45b0bb410");
    REQUIRE(changed_versions.version == 2u);
    REQUIRE_STR(changed_versions.source_drop_id, "835:000000102:102:0001");
...
static int test_member_coverage_aggregate_from_journal(void)
{
    ...
    REQUIRE(outbox_seen.count == 3u);
    REQUIRE_STR(outbox_seen.notifications[0].event_type, "SourceDropAggregatesRecorded");
    REQUIRE_STR(outbox_seen.notifications[0].aggregate_type, "member_coverage");
    REQUIRE_STR(outbox_seen.notifications[0].run_id, "coverage-stitch-test");
    REQUIRE_STR(outbox_seen.notifications[0].source_run_id, "coverage-ingest-test");
    REQUIRE(outbox_seen.notifications[0].aggregate_version_count == 1u);
    REQUIRE(strstr(outbox_seen.notifications[0].payload_json, "SUB-STROKE-001") == NULL);
    REQUIRE(strstr(outbox_seen.notifications[0].payload_json, "PAYOR123") == NULL);
    REQUIRE(strstr(outbox_seen.notifications[0].payload_json, "19790314") == NULL);
    REQUIRE(strstr(outbox_seen.notifications[0].payload_json, "REID") == NULL);
    REQUIRE(strstr(outbox_seen.notifications[0].payload_json, "ALEX") == NULL);
    memset(&changed_versions, 0, sizeof(changed_versions));
    REQUIRE_OK(scribe_store_each_member_coverage_version_for_source_drop(
                &store,
    ...
    REQUIRE(changed_versions.count == 1u);
    REQUIRE_STR(changed_versions.aggregate_id, aggregate_id);
    REQUIRE(changed_versions.version == 3u);
    REQUIRE_STR(changed_versions.source_drop_id, "271:000000111:111:0001");
...
int main(void)
{
    REQUIRE(test_mappers_preserve_phi_mapping_errors() == 0);
