...
need_file demo/stroke_delta.sqlite
need_file demo/stroke_phi_delta.sqlite
...
"
    ...
    echo
    echo "Read-store outbox: tokenised read store"
    sqlite3 -line demo/stroke_read_store.sqlite "
select
  sequence,
  notification_id,
...
"
    ...
    echo
    echo "SQLite delta export: tokenised"
    sqlite3 -line demo/stroke_delta.sqlite "
select key, value
from metadata
order by key;
select
  aggregate_type,
  source_drop_id,
  count(*) as aggregate_versions
from aggregate_versions
group by aggregate_type, source_drop_id
order by aggregate_type, source_drop_id
limit 8;
"

    echo
    echo "SQLite delta export: PHI read store"
    sqlite3 -line demo/stroke_phi_delta.sqlite "
select key, value
from metadata
order by key;
select
