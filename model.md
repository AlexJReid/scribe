...
- Outbox: non-PHI `SourceDropAggregatesRecorded` facts.
...
| Outbox | `outbox_notifications` | Source-drop notifications emitted after aggregate versions are recorded. A separate service owns fan-out to subscribers. | Keeps subscriber delivery/retry/cursor concerns out of the aggregate reducer and avoids one notification per aggregate version. |
...
latest aggregate tables, indexes, and the `outbox_notifications` table.
Consumers poll the outbox by sequence, then ask the private API/read-store
resolver for aggregate versions changed by the notification's `source_drop_id`.
`scribe export delta` packages a sequence window into a separate SQLite file
with `metadata`, `outbox_notifications`, `source_drops`, and normalized
`aggregate_versions` tables. That delta file is a disposable transfer artifact;
the read-store outbox remains the cursorable source of truth.
...
When PHI is resolved into a read store or delta export, Scribe assumes the
handoff is encrypted at the transport/storage layer and receiving systems are
explicitly authorized for PHI. Those systems carry their own auditing, access
control, retention, and operational controls. Scribe audits vault resolution; it
does not replace downstream governance.
